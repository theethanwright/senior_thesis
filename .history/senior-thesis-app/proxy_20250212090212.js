        ...
        (error, response, body) => {
            ...
            if (contentType.includes("text/html")) {
                ...
                modifiedBody = modifiedBody.replace(
                    /(href|src)="(\/[^"]+)"/g,
                    (match, attr, urlPath) => {
                        // Skip rewriting if it's already a proxy URL or a webpack chunk in the scripts directory.
                        if (urlPath.startsWith("/proxy") || urlPath.startsWith("/scripts/")) {
                            return `${attr}="${urlPath}"`;
                        }
                  ...
                  <script>
                    document.addEventListener("click", function(event) {
                      ...
                      if (target && target.tagName === "A") {
                        // If the link is already proxied, let the browser handle it normally.
                        if (target.href.indexOf("/proxy?url=") !== -1) {
                          return;
                        }
