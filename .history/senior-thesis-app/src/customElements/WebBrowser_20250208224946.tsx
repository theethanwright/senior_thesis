...
export function LiveBrowser({ shape }: { shape: BrowserShape }) {
  ...
  // New state and refs for controlling iframe interactions.
  const [iframeDisabled, setIframeDisabled] = useState(false);
  const lastClick = useRef<number | null>(null);
  const timerRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  const handleWrapperClick = () => {
    const now = Date.now();
    if (lastClick.current && now - lastClick.current < 15000) {
      // Double click: re-enable iframe interactions.
      ...
      // Single click: disable interactions.
      setIframeDisabled(true);
      lastClick.current = now;
      timerRef.current = setTimeout(() => {
        // Reset click tracker in case no second click occurs.
        lastClick.current = null;
      }, 15000);
    }
  };
