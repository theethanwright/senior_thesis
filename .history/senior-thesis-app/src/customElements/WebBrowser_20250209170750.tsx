...
import {
  ...
  useEditor,
  TLShapeId,
  track, // imported from tldraw to wrap our component if desired
} from 'tldraw'

type BrowserShape = TLBaseShape<'browser', { w: number; h: number; url: string }>

// Wrap the component with `track` for performance (optional)
export const LiveBrowser = track(function LiveBrowser({ shape }: { shape: BrowserShape }) {
  ...
  // Duplicate the current browser shape when a link is clicked inside the iframe.
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      if (
        !originalIframeRef.current ||
        event.source !== originalIframeRef.current.contentWindow
      ) {
        return
      }
  ...
  // Toggle iframe interaction when double-clicking.
  const handleDoubleClick = () => {
    if (!editor.isIn('select.idle')) return
    if (!editor.getSelectionRotatedPageBounds()) return
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      />
      <div
        // The overlay intercepts double-clicks (and stops pointer events from propagating)
