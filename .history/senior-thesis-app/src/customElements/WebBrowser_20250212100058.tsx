...
export function LiveBrowser({ shape }: { shape: BrowserShape }) {
  ...
  const duplicateBrowser = (clickedUrl: string | undefined) => {
    ...
    let baseUrl = 'http://localhost:8000/proxy?url=';
    console.log("Clicked URL:", newUrl)
    const proxyPrefix = '/proxy?url=';
    if (newUrl.includes(proxyPrefix)) {
      newUrl = decodeURIComponent(newUrl.substring(baseUrl.length))
  ...
  console.log(`http://localhost:8000/proxy?url=${encodeURIComponent(shape.props.url)}`)
